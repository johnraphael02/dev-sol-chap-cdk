...
exports.handler = async (event) => {
    try {
        ...
        const { id, userId, message, status, timestamp } = body;
        if (!id || !userId || !message || !status) {
            console.error("Error: Missing required fields.");
            return { statusCode: 400, body: JSON.stringify({ message: "Missing required fields: id, userId, message, status" }) };
        }

        const createdAt = timestamp || new Date().toISOString();

        // Encrypt PK, SK, message, status, and indexes
