...
class DevSolChapCdkStackStack extends Stack {
  ...
  //  Allow all Lambda functions to send messages to SQS
  ...
  //  Allow all Lambda functions to publish events to EventBridge
  allLambdas.forEach(lambdaFn => {
      lambdaFn.addToRolePolicy(new iam.PolicyStatement({
  ...
  profileReadRule.addTarget(new targets.LambdaFunction(getUserProfile));
    ...
    // 📌 Grant permission to invoke the encryption and decryption Lambdas for relevant functions
    [
      createMarketplaceLambda,
