...
class DevSolChapCdkStackStack extends Stack {
  constructor(scope, id, props) {
    super(scope, id, {
      ...props,
      env: {
        account: process.env.CDK_DEFAULT_ACCOUNT || "0669-2621-7034",
    ...
    const lambdaExecutionRole = new iam.Role(this, "LambdaExecutionRole", {
      ...
      resources: [
        `arn:aws:lambda:${this.region}:${this.account}:function:sol-chap-encryption`,
        `arn:aws:lambda:${this.region}:${this.account}:function:sol-chap-decryption`
      ],
    });


    const subcategoryQueue = new sqs.Queue(this, "SubcategoryQueue", {
      queueName: "SubcategoryQueue",
      visibilityTimeout: cdk.Duration.seconds(30),
    });
    ...
    const deleteMarketplace = new lambda.Function(this, "DeleteMarketplaceLambda", {
      ...
      environment: {
        ...
        EVENT_BUS_NAME: marketplaceEventBus.eventBusArn, // ✅ Add this
        ENCRYPTION_LAMBDA_NAME: "sol-chap-encryption", // ✅ Pass encryption Lambda name
      },
      memorySize: 128,
      timeout: cdk.Duration.seconds(60),
      tracing: lambda.Tracing.ACTIVE,
     ...
     */
     const createMarketplaceLambda = new Function(this, "CreateMarketplaceLambda", {
      ...
      environment: {
          ...
          ENCRYPTION_LAMBDA_NAME: "sol-chap-encryption", // ✅ Pass encryption Lambda name as env variable
      },
    ...
    const getAllMarketplaceLambda = new lambda.Function(this, "GetAllMarketplaceLambda", {
      functionName: "Dev-GetAllMarketplaceFunction",
      runtime: lambda.Runtime.NODEJS_18_X,
      handler: "index.handler",
      code: lambda.Code.fromAsset("lambda/marketplace/dev-getAllMarketplace"),
      environment: {
        MARKETPLACE_TABLE: marketplaceTable.tableName,
        DECRYPTION_LAMBDA_NAME: "sol-chap-decryption", // ✅ Add decryption Lambda reference
      },
      memorySize: 128,
      timeout: cdk.Duration.seconds(60),
      tracing: lambda.Tracing.ACTIVE,
  ...
  marketplaceTable.grantReadData(getAllMarketplaceLambda);
 ...
 // ✅ Allow all Lambda functions to publish events to ALL EventBridge buses
  allLambdas.forEach(lambdaFn => {
    lambdaFn.addToRolePolicy(new iam.PolicyStatement({
        ...
        resources: [
            ...
            `arn:aws:events:${this.region}:${this.account}:event-bus/CategoryUpdateEventBus`, // 📌 Added: Category Events
            `arn:aws:events:${this.region}:${this.account}:event-bus/UserEventBus`, // 📌 Added: User Authentication & Deletion Events
            `arn:aws:events:${this.region}:${this.account}:event-bus/MembershipEventBus`, // 📌 Added: Membership Upgrade Events
            `arn:aws:events:${this.region}:${this.account}:event-bus/BidEventBus` // 📌 Added: Bidding Events
        ]
    }));
  });
  ...
  profileReadRule.addTarget(new targets.LambdaFunction(getUserProfile));
    ...
    // const encryptionLambdaArn = lambda.Function.fromFunctionArn(this, "EncryptionLambda", 
    //   `arn:aws:lambda:${this.region}:${this.account}:function:sol-chap-encryption`
    // );
    
    // const decryptionLambdaArn = lambda.Function.fromFunctionArn(this, "DecryptionLambda", 
    //   `arn:aws:lambda:${this.region}:${this.account}:function:sol-chap-decryption`
    // );
    ...
    // ✅ Allow invoking the decryption Lambda
    getAllMarketplaceLambda.addToRolePolicy(new iam.PolicyStatement({
      actions: ["lambda:InvokeFunction"],
      resources: [`arn:aws:lambda:${this.region}:${this.account}:function:sol-chap-decryption`],
    }));
    

    // ✅ Grant Permission to Invoke Encryption Lambda
    deleteMarketplace.addToRolePolicy(new iam.PolicyStatement({
      actions: ["lambda:InvokeFunction"],
      resources: [`arn:aws:lambda:${this.region}:${this.account}:function:sol-chap-encryption`],
    }));
  ...
  //     allowMethods: apigateway.Cors.ALL_METHODS, // Allow GET, POST, PUT, DELETE, OPTIONS
...
marketplaceResource.addMethod("PUT", new apigateway.LambdaIntegration(updateMarketplaceLambda));
marketplaceResource.addMethod("GET", new apigateway.LambdaIntegration(getAllMarketplaceLambda));


const marketplaceIdResource = marketplaceResource.addResource('{id}');
// ✅ Ensure DELETE API method is correctly added with CORS Headers
marketplaceIdResource.addMethod('DELETE', new apigateway.LambdaIntegration(deleteMarketplace, {
  proxy: true,
...
if (!marketplaceResource.getResource('OPTIONS')) {
